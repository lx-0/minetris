...
function _applyLockToElement(el, mode, playerLevel, unlocked) {
  if (unlocked) {
    ...
    var incentive = _MODE_LOCK_INCENTIVES[mode] || '';
    ...
    if (!el.querySelector('.mode-lock-overlay')) {
      ...
      var incentiveHTML = incentive
        ? '<span class="mode-lock-incentive">' + incentive + '</span>'
        : '';
      overlay.innerHTML = '<span class="mode-lock-icon">&#128274;</span><span class="mode-lock-text">Level ' + reqLevel + '</span>' + incentiveHTML;
